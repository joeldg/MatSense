...
def prepare_cfg():
    ...
    yaml_path = osp.join(osp.dirname(__file__), 'configs/yamls/demo.yaml')
    cfg.merge_from_file(yaml_path)
    # Force GPU layers to run on cpu to avoid deadlocking Apple Silicon MPS wrappers
    cfg.defrost()
    cfg.DEVICE = 'cpu'
    
    # Resolvers for inner paths
    cfg.MODEL_CONFIG = osp.join(osp.dirname(__file__), cfg.MODEL_CONFIG)
    cfg.TRAIN.CHECKPOINT = osp.join(osp.dirname(__file__), cfg.TRAIN.CHECKPOINT)
...
class WHAM_API(object):
    def __init__(self):
        ...
        self.network.eval()
        print("DEBUG: Building DetectionModel...")
        self.detector = DetectionModel(self.cfg.DEVICE.lower())
        print("DEBUG: Building FeatureExtractor...")
        self.extractor = FeatureExtractor(self.cfg.DEVICE.lower())
        print("DEBUG: WHAM_API Init complete.")
        self.slam = None
